...
class icarus:
    def __init__(self, camassem):
        self.ca = camassem
        self.logcrit = self.ca.logcritbase + "[Icarus] "
        self.logerr = self.ca.logerrbase + "[Icarus] "
        self.logwarn = self.ca.logwarnbase + "[Icarus] "
        self.loginfo = self.ca.loginfobase + "[Icarus] "
        self.logdebug = self.ca.logdebugbase + "[Icarus] "
        ...
        self.icarustype = 1  # 2-frame version
    ...
    def setInterlacing(self, ifactor):
        ...
        if ifactor:
            logging.warning(
                self.logwarn + "Interlacing is not supported by the Icarus sensor."
            )
        return 1
    ...
    def setHighFullWell(self, flag):
        ...
        if flag:
            logging.warning(
                self.logwarn + "HighFullWell mode is not supported by the Icarus "
                "sensor. "
            )
    ...
    def setZeroDeadTime(self, flag):
        ...
        if flag:
            logging.warning(
                self.logwarn + "ZeroDeadTime mode is not supported by the Icarus "
                "sensor. "
            )
    ...
    def setTriggerDelay(self, delayblocks):
        ...
        if delayblocks:
            logging.warning(
                self.logwarn + "Trigger Delay mode is not supported by the Icarus "
                "sensor. "
            )
    ...
    def setTiming(self, side, sequence, delay):
        ...
        # all four frames must be managed, even though only two are acquired
        if repeats > 4:
            repeats = 4
        ...
        if repeats < 4:
            ...
            expected = [delay] + list(sequence) + [sequence[0]]
            if actual != expected:
                logging.warning(
                    self.logwarn + "Due to sequence length and use of the Icarus "
                    "model 1 sensor, the actual timing sequence for side "
                    + side
                    + " will be "
                    + "{"
            ...
            logging.warning(
                self.logwarn + "Due to use of the Icarus model 1 sensor, the"
    ...
    def setArbTiming(self, side, sequence):
        ...
        sequence = sequence[:8]  # need all 4 frames to work properly
        for a in sequence:
            add = [flag] * a
            bitlist += add
            if flag:
